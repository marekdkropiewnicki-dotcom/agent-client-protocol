...
mod test_serialization {
    ...
    // (`8659fc2`, "Rename provider method types to singular") shows how
    // a simple rename can shift these strings; the tests below pin
    // every stable variant to its documented wire name so any
    // accidental change surfaces here instead of breaking clients in
    // the wild. Mirror of the equivalent v1 coverage.
    ...
    fn test_client_request_method_routing_for_stable_variants() {
        assert_eq!(
            ClientRequest::InitializeRequest(InitializeRequest::new(ProtocolVersion::V2)).method(),
            "initialize",
        );
        assert_eq!(
